...
/**
 * The genres this screen builds rails from. Animation is deliberately absent —
 * every anime carries it, so it would duplicate the popular rail.
 */
const TARGET_GENRES = [
  'Action & Adventure',
  'Comedy',
...
export default function AnimeDiscovery() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setIsLoading(true)
      try {
        const popularResponse = await apiService.getPopularAnime(1)
        ...
        const genresResponse = await apiService.getAnimeGenres()
        if (!cancelled && genresResponse.success && genresResponse.data) {
          ...
          for (const genre of genres) {
            try {
              const response = await apiService.getAnimeByGenre(genre.id, 1)
  ...
  return (
    <>
      <DiscoveryScreen
        // Anime are TV series: the same cards, modals and request flow apply.
        kind="tv"
        eyebrow="Find"
        title="Discover anime"
        description="Find your next series"
        searchPlaceholder="Search for anime…"
        searchTab="anime"
        featured={featured}
        popular={popular}
        popularTitle="Popular anime"
